"""
SixiD Host Installer
Installs the SixiD agent as an invisible background process.
"""
import ctypes
import os
import shutil
...
# PyInstaller: force include all agent modules
import sixid_agent
import sixid_agent.main
import sixid_agent.config
import sixid_agent.api_client
import sixid_agent.scheduler
import sixid_agent.change_detector
import sixid_agent.collectors
import sixid_agent.collectors.base
import sixid_agent.collectors.bios
import sixid_agent.collectors.cpu
import sixid_agent.collectors.hostname
import sixid_agent.collectors.monitors
import sixid_agent.collectors.motherboard
import sixid_agent.collectors.network
import sixid_agent.collectors.os_info
import sixid_agent.collectors.printers
import sixid_agent.collectors.processes
import sixid_agent.collectors.ram
import sixid_agent.collectors.services
import sixid_agent.collectors.software
import sixid_agent.collectors.storage
import sixid_agent.actions
import sixid_agent.actions.command_executor
import sixid_agent.actions.screen_lock
import sixid_agent.actions.vnc_manager

INSTALL_DIR = os.path.join(os.environ.get("PROGRAMFILES", "C:\\Program Files"), "SixID")
CONFIG_DIR = os.path.join(os.environ.get("PROGRAMDATA", "C:\\ProgramData"), "SixID")
CONFIG_FILE = os.path.join(CONFIG_DIR, "config.ini")
AGENT_EXE_NAME = "SixiDHost.exe"
TASK_NAME = "SixiDAgent"
...
def _stop_previous():
    ...
    subprocess.run(["sc", "stop", "SixiDAgent"], capture_output=True, timeout=10)
    subprocess.run(["sc", "delete", "SixiDAgent"], capture_output=True, timeout=10)
    subprocess.run(["taskkill", "/F", "/IM", AGENT_EXE_NAME], capture_output=True, timeout=10)
    # Also stop old SysID9 if present
    subprocess.run(["schtasks", "/Delete", "/TN", "SysID9Agent", "/F"], capture_output=True, timeout=10)
    ...
    subprocess.run(["taskkill", "/F", "/IM", "SysID9Host.exe"], capture_output=True, timeout=10)
    time.sleep(2)
...
def install(server_url, api_key, progress_callback=None):
    ...
    try:
        ...
        try:
            import winreg
            key_path = r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SixiDAgent"
            key = winreg.CreateKeyEx(winreg.HKEY_LOCAL_MACHINE, key_path, 0, winreg.KEY_WRITE)
            winreg.SetValueEx(key, "DisplayName", 0, winreg.REG_SZ, "SixiD Agent")
            winreg.SetValueEx(key, "UninstallString", 0, winreg.REG_SZ, f'"{agent_exe}" --uninstall')
            winreg.SetValueEx(key, "Publisher", 0, winreg.REG_SZ, "SixiD")
    ...
    try:
        ...
        try:
            ...
            winreg.DeleteKey(
                ...
                r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SixiDAgent",
            )
        except Exception:
            pass
        ...
        ctypes.windll.user32.MessageBoxW(
            0, "SixiD Agent desinstalado com sucesso.", "SixiD", 0x40
        )
    except Exception as e:
        ctypes.windll.user32.MessageBoxW(
            0, f"Erro na desinstalação: {e}", "SixiD", 0x10
        )
    sys.exit(0)
...
def run_agent():
    ...
    from sixid_agent.main import run_as_console
    run_as_console()


def detect_server_url():
    return "http://10.30.200.200:8000"
...
class InstallerGUI:
    ...
    def _build_ui(self):
        ...
        tk.Label(header_inner, text="⬡", font=("Segoe UI", 28),
                 bg=accent, fg="white").pack(side="left", padx=(0, 8))
        title_frame = tk.Frame(header_inner, bg=accent)
        title_frame.pack(side="left")
        tk.Label(title_frame, text="SixiD Agent", font=("Segoe UI", 22, "bold"),
                 bg=accent, fg="white").pack(anchor="w")
        tk.Label(title_frame, text="Sistema de Gestão de Ativos e Inventário de TI",
                 font=("Segoe UI", 9), bg=accent, fg="#B0C4FF").pack(anchor="w")

        # Content
        content = tk.Frame(self.root, bg=bg, padx=40, pady=20)
        content.pack(fill="both", expand=True)

        tk.Label(content, text="Instalação do Agente",
                 font=("Segoe UI", 13, "bold"), bg=bg, fg=fg).pack(pady=(0, 20))

        existing_id = _read_existing_agent_id()
        if existing_id:
            tk.Label(content,
                     text=f"✓ Atualização detectada (ID: {existing_id[:8]}...)",
                     font=("Segoe UI", 9), bg=bg, fg=teal).pack(pady=(0, 10))

        tk.Label(content, text="URL do Servidor:", font=("Segoe UI", 10, "bold"),
        ...
        tk.Label(content, text="Token de Registro:", font=("Segoe UI", 10, "bold"),
                 bg=bg, fg=fg, anchor="w").pack(fill="x")
        ...
        tk.Label(content, text="O agente será invisível e iniciará com o Windows.",
                 font=("Segoe UI", 9, "italic"), bg=bg, fg=gray).pack(anchor="w", pady=(2, 10))

        self.log_text = tk.Text(content, height=5, font=("Consolas", 9),
                                bg="#0F1729", fg=teal, relief="flat",
        ...
        self.install_btn = tk.Button(
            ...
            activebackground="#3D82FF", activeforeground="white",
        ...
        self.cancel_btn = tk.Button(
            ...
            bg="#1E293B", fg=fg, relief="flat", padx=20, pady=8,
